...
describe("ListTemplate", () => {
  it("renders title and content zone", () => {
    render(
      withLayout(
        <ListTemplate title="Contacts" content={<div>DataTable here</div>} />,
      ),
    );
...
describe("AuthTemplate", () => {
  it("renders auth form centered", () => {
    render(<AuthTemplate content={<div>Login form</div>} />);
...
describe("ConversationTemplate", () => {
  it("renders chat content (manual mode)", () => {
    render(
      withLayout(<ConversationTemplate content={<div>chat messages</div>} />),
    );
...
describe("ConversationTemplate (wired mode)", () => {
      ...
      sendMessage: () => {
        ...
        return {
          ...
          onError: (cb: (err: Error) => void) => {
            callbacks.error = cb as (...args: unknown[]) => void;
          },
          abort: () => {},
        };
      },
  ...
  it("renders conversation sidebar and chat engine from adapter", async () => {
    ...
    const { findByText } = render(
      withLayout(<ConversationTemplate adapter={adapter} />),
    );
    ...
    const { findByText } = render(
      withLayout(
        <ConversationTemplate
          adapter={adapter}
          renderContent={(state) => (
            <div>custom content: {state.conversations.length} convs</div>
          )}
        />,
      ),
    );
    ...
    const { findByText } = render(
      withLayout(
        <ConversationTemplate
          adapter={adapter}
          renderSidebar={(state) => (
            <div>custom sidebar: {state.conversations.length}</div>
          )}
        />,
      ),
    );
          ...
          columns={[
            {
              ...
              title: "To Do",
              count: 5,
              children: <div>task cards</div>,
            },
            {
              id: "progress",
              title: "In Progress",
              count: 3,
              children: <div>active tasks</div>,
            },
            {
              id: "done",
              title: "Done",
              count: 12,
              children: <div>completed</div>,
            },
          ]}
        />,
      ),
