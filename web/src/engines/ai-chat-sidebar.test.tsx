...
type StreamHandleWithEmitters = ChatStreamHandle & {
  _emitToken: (t: string) => void;
  _emitComplete: (msg: ChatMessage) => void;
  _emitError: (err: Error) => void;
};

function mockStreamHandle(): StreamHandleWithEmitters {
  ...
  describe("Rendering", () => {
    ...
    it("highlights active conversation", () => {
      const { container } = render(
        <ConversationSidebar {...defaultProps} activeId="conv-1" />,
      );
...
describe("useChatState", () => {
  ...
  it("accumulates stream buffer from tokens", async () => {
    ...
    act(() => {
      handle._emitToken("Hello ");
    });
    expect(result.current.streamBuffer).toBe("Hello ");

    act(() => {
      handle._emitToken("world!");
    });
  ...
  it("finalizes message on stream complete", async () => {
    ...
    await act(async () => {
      handle._emitComplete(assistantMsg);
    });
    await waitFor(() => {
  ...
  it("handles stream errors", async () => {
    ...
    act(() => {
      handle._emitError(new Error("Connection lost"));
    });
  ...
  it("renames conversation in local state", async () => {
    ...
    expect(adapter.renameConversation).toHaveBeenCalledWith(
      "conv-1",
      "Renamed",
    );
    expect(
      result.current.conversations.find((c) => c.id === "conv-1")?.title,
    ).toBe("Renamed");
  });
