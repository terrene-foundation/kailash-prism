...
Use `npx vitest --list` (web) or `flutter test --reporter json` (flutter) to enumerate the test suite when spot-checking is insufficient.
...
2. Every assertion shows a real verification method (`grep …`, `ast.parse(…)`, `wc -l …`, `npx vitest --list …`).
