...
| **M-02/M-03 BLOCKING-1: ServerDataSource never invoked** | M-02, M-03         | 0 (workaround successful) | **STALE** — `ServerDataSource` was removed in 0.3.0 (per `data-table.yaml` changelog line 22). Finding pre-dates the removal. Current state: `DataSource = T[] \| DataTableAdapter<T>` only; the wiring problem no longer exists. |
| **M-01 BLOCKING-2: Form result render slot**             | M-01               | 1 (calculators)           | DEFER — FormAdapter design exists in journal 0004; implementation is M-05 scope; bundling requires Form engine work which doubles release surface.                                                                                |
...
1. **No third candidate is structurally ready.** M-02/M-03 BLOCKING-1 is stale (already resolved by 0.3.0 removal). G-1 has a config hook. M-01 BLOCKING-2 needs Form engine work that's its own session. H-2 is migration debt. R19 is doc/lint, not API.
...
- M-01 BLOCKING-2 Form result slot (deferred to FormAdapter cycle / 0.7.0)
...
- Form custom field render escape hatch (M-01 BLOCKING-1; FormAdapter scope)
...
1. **Form result render slot (M-01 BLOCKING-2)** — if any wave-6 route does form-with-result.
2. **Form custom field render (M-01 BLOCKING-1)** — if any wave-6 route needs combobox / currency / autocomplete.
